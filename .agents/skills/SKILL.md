...
- Skill（技能书）需覆盖：目录结构、命名规范、依赖选型、代码风格、接口/测试/CI/CD/团队约定等
- Skill 建议每项目单独维护，AI 代码生成必须严格遵循
- 包管理工具统一使用 pnpm
...
- 目录结构建议：
  - src/
    ...
    - styles/      # 全局样式
    - lib/         # 工具函数/业务逻辑
- 配置文件（package.json、vite.config.ts、tsconfig.json、uno.config.ts/tailwind.config.js）需规范

## 3. 组件开发规范
- 文件名用 PascalCase，props/事件/slot 明确声明，类型注释齐全
- 复用优先，避免重复造轮子
...
- 页面文件放 routes/，支持嵌套路由，404 页面单独实现
